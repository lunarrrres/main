...
flowers.forEach((flower) => {
    ...
    let selectedVariantId = localStorage.getItem(`selectedVariant-${flower.id}`) || null;
    ...
    seeMore.addEventListener("click", () => {
        ...
        const variant = flowers.find(f => f.id == flower.id).variants.find(v => v.id == selectedVariantId);
            localStorage.setItem(`selectedVariant-${flower.id}`, JSON.stringify(variant));


        window.location.href = `more.html?id=${flower.id}`;
    });
